<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_article_public" style="margin-top: 5px !important;">
      <thead>
          <tr>
                        
          
          <?php include_partial('adArticlesPublish/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_article): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          ...
          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_article->getId() ?>}" rel="<?php echo $ad_article->getId() ?>">
                ...
                <?php include_partial('adArticlesPublish/list_td_tabular', array('ad_article' => $ad_article, 'orderNo' => $orderNo)) ?>
      ...
      <tfoot>
        <tr>
          <th colspan="8">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('adArticlesPublish/pagination', array('pager' => $pager)) ?></div>
