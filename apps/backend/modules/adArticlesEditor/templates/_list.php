<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_articles" style="margin-top: 5px !important;">
      <thead>
          <tr>
                        
          
          <?php include_partial('adArticlesEditor/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_article): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          ...
          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_article->getId() ?>}" rel="<?php echo $ad_article->getId() ?>">

                            
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>d
                <?php include_partial('adArticlesEditor/list_td_tabular', array('ad_article' => $ad_article, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('adArticlesEditor/list_td_actions', array('ad_article' => $ad_article, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="9">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('adArticlesEditor/pagination', array('pager' => $pager)) ?></div>
