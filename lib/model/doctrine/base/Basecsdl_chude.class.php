<?php
// Connection Component Binding
Doctrine_Manager::getInstance()->bindComponent('csdl_chude', 'slave');

/**
 * Basecsdl_chude
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property string $tenchude
 * @property string $gioithieu
 * @property string $anhdaidien
 * @property integer $thutu
 * @property boolean $trangthai
 * 
 * @method string     getTenchude()   Returns the current record's "tenchude" value
 * @method string     getGioithieu()  Returns the current record's "gioithieu" value
 * @method string     getAnhdaidien() Returns the current record's "anhdaidien" value
 * @method integer    getThutu()      Returns the current record's "thutu" value
 * @method boolean    getTrangthai()  Returns the current record's "trangthai" value
 * @method csdl_chude setTenchude()   Sets the current record's "tenchude" value
 * @method csdl_chude setGioithieu()  Sets the current record's "gioithieu" value
 * @method csdl_chude setAnhdaidien() Sets the current record's "anhdaidien" value
 * @method csdl_chude setThutu()      Sets the current record's "thutu" value
 * @method csdl_chude setTrangthai()  Sets the current record's "trangthai" value
 * 
 * @package    Web_Portals
 * @subpackage model
 * @author     ngoctv1
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class Basecsdl_chude extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('csdl_chude');
        $this->hasColumn('tenchude', 'string', 255, array(
             'type' => 'string',
             'comment' => 'Ten chu de',
             'length' => 255,
             ));
        $this->hasColumn('gioithieu', 'string', 500, array(
             'type' => 'string',
             'comment' => 'gioi thieu',
             'length' => 500,
             ));
        $this->hasColumn('anhdaidien', 'string', 255, array(
             'type' => 'string',
             'comment' => 'hinh anh',
             'length' => 255,
             ));
        $this->hasColumn('thutu', 'integer', 5, array(
             'type' => 'integer',
             'comment' => 'Thu tu hien thi',
             'length' => 5,
             ));
        $this->hasColumn('trangthai', 'boolean', null, array(
             'type' => 'boolean',
             'notnull' => true,
             'default' => false,
             'comment' => 'Trang thai',
             ));

        $this->option('collate', 'utf8_general_ci');
        $this->option('charset', 'utf8');
        $this->option('type', 'InnoDB');
    }

    public function setUp()
    {
        parent::setUp();
        $vtblameable0 = new Doctrine_Template_VtBlameable();
        $timestampable0 = new Doctrine_Template_Timestampable();
        $this->actAs($vtblameable0);
        $this->actAs($timestampable0);
    }
}